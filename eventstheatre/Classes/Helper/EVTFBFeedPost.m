...
- (void) publishPostWithDelegate:(id) _delegate {
	...
	BOOL loggedIn = [[EVTFBRequestWrapper defaultManager] isLoggedIn];
	if (!loggedIn) {
		[[EVTFBRequestWrapper defaultManager] FBSessionBegin:self];
	}
	else {
		...
		[[EVTFBRequestWrapper defaultManager] sendFBRequestWithGraphPath:graphPath 
                                                                  params:params 
                                                             andDelegate:self];
	}
}
...
- (void)fbDidLogin {
	[[EVTFBRequestWrapper defaultManager] setIsLoggedIn:YES];
...
- (void)fbDidNotLogin:(BOOL)cancelled {
	[[EVTFBRequestWrapper defaultManager] setIsLoggedIn:NO];
}
